...
function VotingStatusDisplay({ winner, message, votingDeadline, type }) {
  return (
    <div className={`mb-6 p-4 rounded-lg shadow-sm ${type === 'sunday' ? 'bg-teal-100' : 'bg-green-100'}`}>
      <h3 className={`text-xl font-bold mb-2 ${type === 'sunday' ? 'text-teal-700' : 'text-green-700'}`}>
        {type === 'sunday' ? 'Sunday Voting Status' : 'Flexible Voting Status'}
      </h3>
      {winner ? (
        <p className={`text-lg font-bold text-center animate-pulse ${type === 'sunday' ? 'text-teal-800' : 'text-green-800'}`}>
          {message}
        </p>
      ) : (
